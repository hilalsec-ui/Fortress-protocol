/**
 * ═══════════════════════════════════════════════════════════════════════════
 *                       YPL (YEARLY POOL LOTTERY) STRESS TEST
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * MISSION: Validate the complete YPL lottery cycle under stress conditions
 * 
 * YPL CHARACTERISTICS:
 * - Time-based trigger: Winner drawn yearly (every 365 days)
 ...
 * 4. Trigger draw_ypl_winner for each tier
...
describe("YPL STRESS TEST - Yearly Pool Lottery", function() {
  ...
  before(async () => {
    logSeparator("INITIALIZING YPL STRESS TEST");
    ...
    console.log(`\n  YPL PARAMETERS:`);
    console.log(`    Tiers: ${YPL_TIERS.join(", ")}`);
    console.log(`    Draw Trigger: Yearly (every 365 days)`);
  ...
  function getParticipantPagePDA(tier: number, pageNumber: number): [PublicKey, number] {
    return PublicKey.findProgramAddressSync(
      [
        Buffer.from("page"),
        Buffer.from([LOTTERY_TYPE_YPL, 0, 0, 0]),
  ...
  async function buyTicket(tier: number): Promise<string> {
    ...
    const tx = await program.methods
      .buyYplTicket(tier, 1, maxFptAmount, expectedPage)
      .accounts({
  ...
  for (const tier of YPL_TIERS) {
    describe(`YPL TIER ${tier} ($${tier} USD)`, function() {
      ...
      it(`should check draw eligibility (time-based)`, async function() {
        ...
        if (currentTime >= endTime) {
          ...
          console.log(`  ⏳ Draw NOT eligible yet (${daysRemaining} days remaining)`);
          console.log(`  Note: In production, wait for yearly trigger. For testing, admin can override.`);
        }
      });
  ...
  after(async () => {
    logSeparator("YPL STRESS TEST COMPLETE");
    
    console.log("\n  SUMMARY:");
    console.log(`    Tiers Tested: ${YPL_TIERS.join(", ")}`);
    console.log(`    Tickets per Tier: ${PARTICIPANTS_TO_TEST}`);
    console.log(`    Pages per Tier: ${Math.ceil(PARTICIPANTS_TO_TEST / PAGE_SIZE)}`);
    console.log(`    Draw Trigger: Yearly (time-based)`);
