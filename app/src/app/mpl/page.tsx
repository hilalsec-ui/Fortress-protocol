...
const MPLPage: React.FC = () => {
  ...
  const { phases: onChainPhase, isInitialized: onChainReady, requestedTimes, userInitiated } = usePendingDraws(program, 'MPL', [5, 10, 15, 20]);
  ...
  const lotteryData = useMemo(() => {
    ...
    const mplData = lotteryAccounts.find((l: any) => l.lotteryType === 'MPL');
    if (!mplData) return null;
    return {
      lotteryType: 'MPL',
      title: 'Monthly Pool',
      description: 'Bigger pools, bigger prizes - accumulation creates value',
      currentParticipants: mplData.currentParticipants || 0,
      lastWinner: mplData.lastWinner || null,
      // Only render tiers that actually exist on-chain — no ghost placeholders
      tiers: mplData.tiers?.length > 0 ? mplData.tiers : [],
    };
  }, [lotteryAccounts]);
  ...
  // Auto-trigger house draw when time expires for any MPL tier (no wallet popup).
      ...
      if (expired && hasParticipants && onChainReady && getPhase(t) === 'idle' && !houseTriggerFiredRef.current.has(key)) {
        ...
        (async () => {
          ...
          const doFulfill = async (): Promise<boolean> => {
            try {
              const fRes = await fetch('/api/draw/fulfill', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lottery_type: 'MPL', tier: t }) });
              const fData = await fRes.json();
              if (fData.success) {
                toast.success(`🏆 Draw complete! Winner: ${String(fData.winner ?? '').slice(0, 6)}…`, { duration: 6000 });
                setWinnerData({ wallet: fData.winner, tier: t, prize: ((fData.prize ?? 0) / 1_000_000) * 0.95, lotteryType: 'MPL' });
                setShowCelebration(true);
                return true;
              }
              if (fData.already_fulfilled) return true;
              console.warn(`[MPL] Fulfill: ${fData.error ?? 'unknown'} — fallback will activate`);
              return false;
            } catch (e) { console.error('[MPL] Fulfill error:', e); return false; }
          };
          for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
              const res = await fetch('/api/draw/request', {
                ...
                body: JSON.stringify({ lottery_type: 'MPL', tier: t }),
              });
              const data = await res.json();
              if (data.fallback_required) {
                console.warn(`[MPL] Crank balance too low on attempt ${attempt}/${MAX_ATTEMPTS} — will exhaust retries then show user fallback`);
                ...
                console.warn(`[MPL] Oracle failed on attempt ${attempt}/${MAX_ATTEMPTS} — kicking oracle directly`);
                try {
                  ...
                  if (orbData.fallback_required) {
                    console.warn(`[MPL] Oracle crank low on attempt ${attempt}/${MAX_ATTEMPTS} — will exhaust retries then show user fallback`);
                  }
                } catch (orbErr) {
                  console.error(`[MPL] Oracle kick failed on attempt ${attempt}/${MAX_ATTEMPTS}:`, orbErr);
                }
                ...
                console.warn(`[MPL] Retrying oracle for existing PendingDraw on attempt ${attempt}/${MAX_ATTEMPTS}`);
                try {
                  const orbRes = await fetch('/api/draw/oracle', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                  ...
                  console.error(`[MPL] Oracle retry kick failed on attempt ${attempt}/${MAX_ATTEMPTS}:`, orbErr);
                }
              } else {
                console.warn(`[MPL] House draw attempt ${attempt}/${MAX_ATTEMPTS} failed:`, data.error);
              }
            } catch (err) {
              console.error(`[MPL] House draw attempt ${attempt}/${MAX_ATTEMPTS} network error:`, err);
            }
          ...
          if (succeeded) {
            ...
            console.warn('[MPL] All 3 draw attempts failed — user fallback required');
  ...
  const handleSettle = async (tier: number) => {
    ...
    try {
      ...
      const result = await fulfillDrawEntropy(program, 'MPL', tier, publicKey);
      ...
      setWinnerData({ wallet: result.winner, tier, prize: result.prize, lotteryType: 'MPL' });
      setShowCelebration(true);
      refresh();
    } catch (error: any) {
      toast.dismiss(`draw-settle-${tier}`);
  ...
  const handleResetAndBuy = async (tier: number) => {
    ...
    try {
      ...
      const result = await buyTicketWithProgram(program, 'MPL', tier, Date.now(), 1, publicKey, undefined, sendTransaction);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-500/10 to-red-500/10">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
          ...
          <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Loading Monthly Pool...</div>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden py-16 px-4 sm:px-8">
        <div className="absolute top-20 right-20 w-72 h-72 bg-orange-500/15 rounded-full blur-[120px]" />
        <div className="absolute bottom-20 left-20 w-96 h-96 bg-red-500/15 rounded-full blur-[120px]" />
        ...
        >
          <div className="text-center mb-8">
            ...
            >
              <div
                className="w-24 h-24 bg-gradient-to-br from-orange-500 to-red-500 rounded-2xl flex items-center justify-center shadow-2xl shadow-orange-500/25"
              >
                <Calendar className="w-12 h-12 text-white" />
              </div>
            </motion.div>
            
            <h1 className={`text-5xl md:text-7xl font-black mb-4 ${isDarkMode ? 'bg-gradient-to-r from-orange-400 to-red-400 bg-clip-text text-transparent' : 'bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent'}`}>
              📅 Monthly Pool
            </h1>
            <p className={`text-2xl md:text-3xl mb-4 ${isDarkMode ? 'text-orange-300' : 'text-orange-700'}`}>
              Bigger Pools, Bigger Prizes • Premium Monthly Draws
            </p>
            <p className={`text-lg max-w-3xl mx-auto ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              The power of accumulation! 30 days of entries create substantial prize pools and exceptional winning opportunities.
            </p>
          </div>
          ...
          <motion.div
            ...
            className={`max-w-4xl mx-auto rounded-3xl p-8 shadow-2xl ${
              isDarkMode ? 'bg-black/40 backdrop-blur-xl border border-orange-500/20' : 'bg-white/80 backdrop-blur-xl'
            }`}
          >
            {/* Tier Selection */}
            ...
            <div className="grid md:grid-cols-4 gap-6 mb-6 relative">
              <div className="text-center">
                <div className="text-5xl font-bold mb-2 text-orange-500">{tierParticipants}</div>
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-amber-500/70 mb-1 tabular-nums">
            ...
            {(() => {
              return (
                <>
                  <div className={`mt-6 p-4 rounded-xl ${isDarkMode ? 'bg-orange-500/10 border border-orange-500/20' : 'bg-orange-50'}`}>
                    <motion.div
                      animate={isDeadPool || tierEndTime === 0 ? {} : { x: [0, 10, 0] }}
                      transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                      className={`text-center mb-2 ${isDarkMode ? 'text-orange-300' : 'text-orange-600'}`}
                    >
      ...
      <section className="py-12 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-6 mb-12">
          {[
            { icon: TrendingUp, title: "Bigger Prize Pools", description: "30 days of accumulation — 48 premium draws per year.", color: "from-orange-500 to-red-500" },
            { icon: Award, title: "Premium Prizes", description: `Auto-triggered by house crank. Finalize for FPT bounty. Winner receives ${PRIZE_WINNER_PCT}% instantly.`, color: "from-red-500 to-orange-600" },
            { icon: Zap, title: "Monthly Events", description: "Substantial monthly pools worth the wait across all tiers.", color: "from-orange-400 to-red-400" }
          ].map((feature, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <div className={`w-14 h-14 bg-gradient-to-br ${feature.color} rounded-xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-orange-500/20`}>
              ...
              >
                ...
                <div className="relative text-center">
                  ...
                  <div className={`-mx-6 -mt-6 mb-4 px-4 py-2 flex items-center justify-center rounded-t-2xl text-xs font-mono font-bold tabular-nums ${
                    tier.endTime === 0
                      ? isDarkMode ? 'bg-orange-500/5 text-gray-500' : 'bg-gray-50 text-gray-400'
                  ...
                  <div className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-red-500 mb-1">
                    ${tier.tier}
                  </div>
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-orange-400/70 tabular-nums">{Number(tier.prizePool).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} FPT</div>
                    ...
                    tier.participants > 0 ? (
                      connected ? (
                        ...
                        <button
                          ...
                          className="w-4/5 mx-auto block py-3 px-4 rounded-xl font-bold transition-all duration-300 transform bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white shadow-lg hover:shadow-xl hover:scale-105"
                        >
                          🔒 Connect Wallet
                        </button>
                      )
