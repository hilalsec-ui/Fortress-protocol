...
import { Sun, Clock, Shield, Sparkles, RefreshCw, ExternalLink } from 'lucide-react';
...
const DPLPage: React.FC = () => {
  ...
  const [localTxState, setLocalTxState] = useState<Record<number, 'house_triggering' | 'settling' | 'fallback_needed' | 'step1_complete' | 'step2_processing' | 'requesting' | null>>({});

  // On-chain draw phase per tier (idle → requested → oracle_ready)
  const { phases: onChainPhase, isInitialized: onChainReady, requestedTimes, userInitiated } = usePendingDraws(program, 'DPL', [5, 10, 15, 20]);
  ...
  const lotteryData = useMemo(() => {
    ...
    const dplData = lotteryAccounts.find((l: any) => l.lotteryType === 'DPL');
    if (!dplData) return null;
    return {
      lotteryType: 'DPL',
      title: 'Daily Pool',
      description: 'Fresh draws every 24 hours',
      currentParticipants: dplData.currentParticipants || 0,
      lastWinner: dplData.lastWinner || null,
      // Only render tiers that actually exist on-chain — no ghost placeholders
      tiers: dplData.tiers?.length > 0 ? dplData.tiers : [],
    };
  }, [lotteryAccounts]);
  ...
  // the UI flips from countdown to "DRAW READY". Without this, accumulated local
  // clock drift (especially after device sleep) can cause a multi-second mismatch.
  const preExpirySyncedRef = useRef<Set<string>>(new Set());
  // Tracks (tier:endTime) keys that have already fired a house trigger this round.
  // Using the endTime as part of the key means a new round (fresh endTime) always
  // gets its own trigger — but the same round never fires twice.
  ...
  useEffect(() => {
    ...
    for (const tier of lotteryData.tiers) {
      ...
      // Trigger once when ≤30 s remain and the timer hasn't expired yet.
  ...
  const handleBuyTicketSubmit = async (tier: number, participantId: number, quantity: number = 1, onProgress?: (step: number, total: number, qty: number) => void) => {
    ...
    const result = await buyTicketWithProgram(program, 'DPL', tier, participantId, quantity, publicKey, onProgress, sendTransaction ?? undefined);
    ...
    if (tierState && tierState.endTime > 0 && nowSeconds >= tierState.endTime && (tierState.participants ?? 0) + quantity > 0) {
      settlementTrigger('DPL', tier, tierState.endTime);
    }

    return result;
  };

  // Auto-trigger house draw when a DPL tier expires with participants.
  // Retries up to 3× with 2 s pauses. If all fail, sets 'fallback_needed'
  // so the user can sign the VRF request manually with their own wallet.
  useEffect(() => {
    if (!lotteryData?.tiers) return;
    for (const tierData of lotteryData.tiers) {
      ...
      // Key is unique per (tier, round): when a new round starts the endTime
      // changes, producing a fresh key — so each round triggers exactly once.
      const triggerKey = `${t}:${tierData.endTime}`;
      if (expired && tierData.participants > 0 && onChainReady && getPhase(t) === 'idle' && !houseTriggerFiredRef.current.has(triggerKey)) {
        houseTriggerFiredRef.current.add(triggerKey); // lock for this round — never deleted
        triggerCrank('DPL', t); // dispatch GitHub crank immediately on expiry
        ...
        (async () => {
          ...
          const doFulfill = async (): Promise<boolean> => {
            try {
              const fRes = await fetch('/api/draw/fulfill', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lottery_type: 'DPL', tier: t }) });
              const fData = await fRes.json();
              if (fData.success) {
                toast.success(`🏆 Draw complete! Winner: ${String(fData.winner ?? '').slice(0, 6)}…`, { duration: 6000 });
                setWinnerData({ wallet: fData.winner, tier: t, prize: ((fData.prize ?? 0) / 1_000_000) * 0.95, lotteryType: 'DPL' });
                setShowCelebration(true);
                return true;
              }
              if (fData.already_fulfilled) return true;
              console.warn(`[DPL] Fulfill: ${fData.error ?? 'unknown'} — fallback will activate`);
              return false;
            } catch (e) { console.error('[DPL] Fulfill error:', e); return false; }
          };
          for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
              const res = await fetch('/api/draw/request', {
                ...
                body: JSON.stringify({ lottery_type: 'DPL', tier: t }),
              });
              const data = await res.json();
              if (data.fallback_required) {
                console.warn(`[DPL] Crank balance too low on attempt ${attempt}/${MAX_ATTEMPTS} — will exhaust retries then show user fallback`);
                ...
                console.warn(`[DPL] Oracle failed on attempt ${attempt}/${MAX_ATTEMPTS} — kicking oracle directly`);
                try {
                  ...
                  if (orbData.fallback_required) {
                    console.warn(`[DPL] Oracle crank low on attempt ${attempt}/${MAX_ATTEMPTS} — will exhaust retries then show user fallback`);
                  }
                } catch (orbErr) {
                  console.error(`[DPL] Oracle kick failed on attempt ${attempt}/${MAX_ATTEMPTS}:`, orbErr);
                }
                ...
                console.warn(`[DPL] Retrying oracle for existing PendingDraw on attempt ${attempt}/${MAX_ATTEMPTS}`);
                try {
                  const orbRes = await fetch('/api/draw/oracle', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                  ...
                  console.error(`[DPL] Oracle retry kick failed on attempt ${attempt}/${MAX_ATTEMPTS}:`, orbErr);
                }
              } else {
                console.warn(`[DPL] House draw attempt ${attempt}/${MAX_ATTEMPTS} failed:`, data.error);
              }
            } catch (err) {
              console.error(`[DPL] House draw attempt ${attempt}/${MAX_ATTEMPTS} network error:`, err);
            }
          ...
          if (succeeded) {
            ...
            console.warn('[DPL] All 3 house draw attempts failed — user fallback required');
            setLocalTxState(prev => ({ ...prev, [t]: 'fallback_needed' }));
          }
          // Note: triggerKey intentionally NOT removed from firedRef — prevents re-fire for this round.
        })();
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lotteryData, nowSeconds, onChainReady]);
  ...
  const handleDrawStep = async (tier: number) => {
    ...
    try {
      ...
      const LOTTERY_TYPE_ID_VAL = 1; // DPL
      ...
      if (pdInfo && pdInfo.data.length >= 42) {
        ...
        const commitRes = await fetch('/api/draw/oracle', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ lottery_type: 'DPL', tier, user_pubkey: publicKey.toBase58(), mode: 'build' }),
        });
        ...
        const revealRes = await fetch('/api/draw/oracle', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ lottery_type: 'DPL', tier, user_pubkey: publicKey.toBase58(), mode: 'build_reveal' }),
        });
      ...
      const initRes = await fetch('/api/draw/init', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ lottery_type: 'DPL', tier, user_pubkey: publicKey.toBase58() }),
      });
      ...
      try {
        const kickRes = await fetch('/api/draw/oracle', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ lottery_type: 'DPL', tier }),
        });
  ...
  // Click 2: Settle — user earns FPT reward, wallet popup required.
  const handleSettle = async (tier: number) => {
    ...
    try {
      toast.loading('⚡ Settling draw & claiming FPT reward…', { id: `draw-settle-${tier}` });
      const result = await fulfillDrawEntropy(program, 'DPL', tier, publicKey);
      toast.dismiss(`draw-settle-${tier}`);
      toast.success(`🏆 Draw settled! FPT reward sent to your wallet! Tx: ${result.signature.slice(0, 8)}…`, { duration: 7000 });
      setWinnerData({ wallet: result.winner, tier, prize: result.prize, lotteryType: 'DPL' });
      setShowCelebration(true);
      refresh();
    } catch (error: any) {
      toast.dismiss(`draw-settle-${tier}`);
  ...
  const handleResetAndBuy = async (tier: number) => {
    ...
    try {
      ...
      const result = await buyTicketWithProgram(program, 'DPL', tier, Date.now(), 1, publicKey, undefined, sendTransaction);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-500/10 to-cyan-500/10">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
          ...
          <div className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Loading Daily Pool...</div>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden py-16 px-4 sm:px-8">
        <div className="absolute top-20 right-20 w-72 h-72 bg-blue-500/15 rounded-full blur-[120px]" />
        <div className="absolute bottom-20 left-20 w-96 h-96 bg-cyan-500/15 rounded-full blur-[120px]" />
        ...
        >
          <div className="text-center mb-8">
            ...
            >
              <div
                className="w-24 h-24 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-2xl flex items-center justify-center shadow-2xl shadow-blue-500/25"
              >
                <Sun className="w-12 h-12 text-white" />
              </div>
            </motion.div>
            
            <h1 className={`text-5xl md:text-7xl font-black mb-4 ${isDarkMode ? 'bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent' : 'bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent'}`}>
              ☀️ Daily Pool
            </h1>
            <p className={`text-2xl md:text-3xl mb-4 ${isDarkMode ? 'text-blue-300' : 'text-blue-700'}`}>
              Fresh Draws Every 24 Hours • Daily Excitement!
            </p>
            <p className={`text-lg max-w-3xl mx-auto ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Every 24 hours the draw fires automatically. If automation ever stalls, any community member can step in as a fallback and instantly earn a <span className="font-bold text-blue-400">FPT bounty</span>. 100% automatic, 100% transparent, 100% on-chain.
            </p>
          </div>
          ...
          <motion.div
            ...
            className={`max-w-4xl mx-auto rounded-3xl p-8 shadow-2xl ${
              isDarkMode ? 'bg-black/40 backdrop-blur-xl border border-blue-500/20' : 'bg-white/80 backdrop-blur-xl'
            }`}
          >
            {/* Tier Selection */}
            ...
            <div className="grid md:grid-cols-4 gap-6 mb-6 relative">
              <div className="text-center">
                <div className="text-5xl font-bold mb-2 text-blue-500">{tierParticipants}</div>
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-cyan-500/70 mb-1 tabular-nums">
            ...
            {(() => {
              return (
                <>
                  <div className={`mt-6 p-4 rounded-xl ${isDarkMode ? 'bg-blue-500/10 border border-blue-500/20' : 'bg-blue-50'}`}>
                    <motion.div
                      animate={isDeadPool || tierEndTime === 0 ? {} : { x: [0, 10, 0] }}
                      transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                      className={`text-center mb-2 ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`}
                    >
      ...
      <section className="py-12 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-6 mb-12">
          {[
            { icon: Clock, title: "24-Hour Rounds", description: "Fresh draws every 24 hours — 1,460 per year across all tiers.", color: "from-blue-500 to-cyan-500" },
            { icon: Shield, title: "Community Draw", description: `Auto-triggered by the house crank. Once Switchboard VRF commits (~2s), anyone can finalize and earn FPT. Winner receives ${PRIZE_WINNER_PCT}% instantly.`, color: "from-cyan-500 to-blue-600" },
            { icon: Sparkles, title: "Daily Winners", description: "Multiple winners every single day across four independent tiers.", color: "from-blue-400 to-cyan-400" }
          ].map((feature, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              <div className={`w-14 h-14 bg-gradient-to-br ${feature.color} rounded-xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-500/20`}>
              ...
              {(lotteryData?.tiers ?? []).map((tier: any, index: number) => (
                ...
                <div className="relative text-center">
                  ...
                  <div className={`-mx-6 -mt-6 mb-4 px-4 py-2 flex items-center justify-center rounded-t-2xl text-xs font-mono font-bold tabular-nums ${
                    tier.endTime === 0
                      ? isDarkMode ? 'bg-blue-500/5 text-gray-500' : 'bg-gray-50 text-gray-400'
                  ...
                  <div className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-cyan-500 mb-1">
                    ${tier.tier}
                  </div>
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-blue-400/70 tabular-nums">{Number(tier.prizePool).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} FPT</div>
                    ...
                    tier.participants > 0 ? (
                      connected ? (
                        ...
                        <button
                          ...
                          className="w-9/10 mx-auto block py-3 px-4 rounded-xl font-bold transition-all duration-300 transform bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white shadow-lg hover:shadow-xl hover:scale-105"
                        >
                          🔒 Connect Wallet
                        </button>
                      )
                          ...
                          <button
                            ...
                            className={`w-9/10 mx-auto block py-3 px-4 rounded-xl font-bold transition-all duration-300 transform ${
