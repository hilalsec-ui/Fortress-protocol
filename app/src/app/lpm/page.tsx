...
import { Zap, Timer, Shield, Sparkles, RefreshCw, Crown } from 'lucide-react';
...
import { LPM_TIERS, SB_RANDOMNESS_ACCOUNTS, PROGRAM_ID, PRIZE_WINNER_PCT } from '@/utils/constants';
...
const LPLPage: React.FC = () => {
  ...
  const resetTier = async (tier: number) => {
    try {
      ...
      console.log(`✅ Tier $${tier} reset requested - requires on-chain transaction`);
    } catch (error) {
  ...
  const handleDrawStep = async (tier: number) => {
    ...
    try {
      ...
      // Set step1_complete so the step tracker immediately shows step 2 button
  ...
  const handleBuyTicketSubmit = async (tier: number, participantId: number, quantity: number = 1, onProgress?: (step: number, total: number, qty: number) => void) => {
    console.log('🎫 handleBuyTicketSubmit called:', { tier, participantId, quantity, connected, publicKey: publicKey?.toString() });
    ...
    console.log('✅ Pre-checks passed, calling buyTicketWithProgram...');
